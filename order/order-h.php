  ...
  ?>
    <section class="order-history-wrapper pt-90 pb-100">
      <div class="container">
        ...
        <?php
          $user_check_query = "SELECT OD.id,OD.Odate FROM orders_detail OD , history H WHERE OD.id = H.Oid";
          $user_check_query2 = "SELECT S.stetus FROM stetuss S , history H WHERE S.id = H.Sid";
          $user_check_query3 = "SELECT P.Pprice FROM orders_detail OD , product P,history H WHERE OD.id = H.Oid AND OD.Pid = P.Pid";
          $query = mysqli_query($dbcon,$user_check_query);      
          $query2 = mysqli_query($dbcon,$user_check_query2); 
          $query3 = mysqli_query($dbcon,$user_check_query3);
          while(($result = mysqli_fetch_array($query)) and ($result2 = mysqli_fetch_array($query2)) and ($result3 = mysqli_fetch_array($query3))){
        ?>
        <div class="row justify-content-center">
          <div class="col-lg-9">
            <div class="single-order">
            <a class="product" href="order-item.php?id=<?=$result['id']?>"><h4 class="order-id">Order ID: <?=$result['id']?></h4></a>
              <ul class="order-meta">
                <li><a class="date" href="order-item.php">วันที่สั่งซื้อ:<?=$result['Odate']?></a></li>
              </ul>
              <div class="single-progress-bar-horizontal shipped">
                สถานะสินค้า: <span id="status"><?=$result2['stetus']?></span>
                <div class="progress-text">
                  <p><span id="status"> </span></p>
                </div>
              </div>
              <span class="order-price"><?=$result3['Pprice']?>บาท</span>
            </div>
          </div>
        </div>
        <?php
